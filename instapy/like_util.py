...
def get_links_from_feed(browser, amount, num_of_search, logger):
    ...
    try:
        ...
        logger.error(f"link_elems error {str(e)}")
...
def get_main_element(browser, link_elems, skip_top_posts):
    ...
    if not link_elems:
        ...
        return (
            ...
            if skip_top_posts
            else browser.find_element_by_tag_name("main")
        )
...
def get_links_for_location(
    ...
    logger.info(
        f'desired amount: {amount}  |  top posts [{"enabled" if not skip_top_posts else "disabled"}]: {len(top_posts)}  |  possible posts: {possible_posts}'
    )
    ...
    if possible_posts is not None:
        ...
        amount = min(amount, possible_posts)
...
def get_links_for_username(
    ...
    if posts_count is not None and amount > posts_count:
        logger.info(
            f"You have requested to get {amount} posts from {person}'s profile page BUT there only {posts_count} posts available :D"
        )
        amount = posts_count
    ...
    while len(links) < amount:
        ...
        main_elem = browser.find_element_by_tag_name("article")
        links += get_links(browser, person, logger, media, main_elem)
        links = sorted(set(links), key=links.index)

        if len(links) == len(initial_links):
            if attempt >= 7:
                logger.info(
                    f"There are possibly less posts than {amount} in {person}'s profile page!"
                )
                break
            else:
                attempt += 1
