...
def check_browser(browser, logfolder, logger, proxy_address):
    ...
    try:
        ...
        if (
            ...
            logger.warn(
                f'- Expected Proxy IP is "{proxy_address}", and the current IP is "{current_ip_info["ip"]}"'
            )
            ...
            logger.info(
                f"""- Current IP is "{current_ip_info["ip"]}" and it\'s from "{current_ip_info["country"]}/{current_ip_info["country_code"]}\""""
            )
            update_activity(
                browser,
                action=None,
